...
create index shared_habits_habit_id_idx on shared_habits(habit_id);
create index shared_habits_owner_id_idx on shared_habits(owner_id);
create index shared_habits_invited_user_id_idx on shared_habits(invited_user_id);
create index shared_habits_status_idx on shared_habits(status);
...
begin
  ...
  if shared_users is not null then
    insert into notifications (user_id, type, from_user_id, content, metadata)
    select
      unnest(shared_users),
