...
import { useChallengeDiscussions, useChallengeDiscussionComments } from '@/hooks/useChallengeDiscussions'
...
export default function ChallengeDiscussions({ challengeId, isCreator }: ChallengeDiscussionsProps) {
  const { user } = useAuth()
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    try {
      ...
      const fileName = `challenge-discussions/${user.id}_${Date.now()}.${fileExt}`
  ...
  const handleSaveEdit = () => {
    ...
    updateDiscussion({ discussionId: editingId, content: editContent.trim(), challengeId })
    setEditingId(null)
    setEditContent('')
  }
  ...
  const handleConfirmDelete = () => {
    if (discussionToDelete) {
      deleteDiscussion({ discussionId: discussionToDelete, challengeId })
      setDiscussionToDelete(null)
    }
  }

  const handleTogglePin = (discussionId: string, isPinned: boolean) => {
    togglePin({ discussionId, isPinned: !isPinned, challengeId })
  }

  const handleToggleReaction = (discussionId: string, userHasReacted: boolean) => {
    if (userHasReacted) {
      removeReaction({ discussionId, challengeId })
    } else {
      addReaction({ discussionId, challengeId })
    }
  }
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            <Textarea
              ...
              placeholder="Share your progress, ask questions, or encourage others..."
      ...
      {discussions && discussions.length > 0 ? (
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-muted-foreground">
            No discussions yet. Be the first to share your progress!
          </p>
        </Card>
      )}
...
function DiscussionCommentsSection({
  ...
  const {
    ...
    deleteComment,
    isCreatingComment,
  } = useChallengeDiscussionComments(discussionId, challengeId)
