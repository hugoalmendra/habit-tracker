...
interface EditGroupModalProps {
  ...
  group: FollowerGroup | null
}

export default function EditGroupModal({ open, onOpenChange, group }: EditGroupModalProps) {
  ...
  const { updateGroup, addMembers, removeMember, useGroupMembers, isUpdating } = useFollowerGroups()

  const { data: currentMembers } = useGroupMembers(group?.id || null)
  ...
  useEffect(() => {
    if (group && open) {
      ...
      // Set initially selected members from current group members
      if (currentMembers) {
        setSelectedMembers(currentMembers.map(m => m.follower_id))
      }
    }
  }, [group, open, currentMembers])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentMemberIds = currentMembers?.map(m => m.follower_id) || []
      const membersToAdd = selectedMembers.filter(id => !currentMemberIds.includes(id))
      const membersToRemove = currentMembers?.filter(m => !selectedMembers.includes(m.follower_id)) || []

      // Add new members
      if (membersToAdd.length > 0) {
        await addMembers({
          groupId: group.id,
          followerIds: membersToAdd
        })
      }
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-semibold tracking-tight">Edit Group</h2>
                    <p className="text-sm text-muted-foreground">Update group details and members</p>
                  </div>
                </div>
